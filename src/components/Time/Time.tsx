...
function Time({ greeting, currentHour, currentMinutes }: TimeProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const locationRes = await fetch('https://api.ipbase.com/v1/json/'); // Use the proxy URL
        if (!locationRes.ok) {
          throw new Error('Failed to fetch location data');
        }
        const locationData = await locationRes.json();
        setLocation({
          regionName: locationData.region_name,
          regionCountryCode: locationData.country_code
        });
      } catch (err) {
  ...
  return (
    <section className="main">
      ...
      <div className="time">
        <p className="hour-minutes">
          {currentHour}:{currentMinutes}
        </p>
        <span className="gtm">GTM</span>
      </div>
      <p className="location">
        in {location.regionName}, {location.regionCountryCode}
      </p>
    </section>
  );
}
